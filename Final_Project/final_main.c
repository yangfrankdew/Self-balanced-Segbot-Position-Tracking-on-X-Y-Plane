...
__interrupt void SWI_isr(void) {
    ...
    vel_WhlDiff = 0.333 * vel_WhlDiff_1 + 2.0/(3.0*0.004)*(WhlDiff - WhlDiff_1);
    eDiff = turnref - WhlDiff;
    intDiff = intDiff_1 + 0.002 * (eDiff + eDiff_1);

    turn = Kp*eDiff + Ki*intDiff - Kd*vel_WhlDiff;
    ...
    v_l = 0.6 * v_l_1 + 100 * (LeftWheel - LeftWheel_1);
    v_r = 0.6 * v_r_1 + 100 * (RightWheel - RightWheel_1);

    Ubal = -K1 * tilt_value - K2 * gyro_value - K3 * (v_l + v_r)/2.0;
...
__interrupt void ADCA_ISR (void)
{
    ...
    SpibRegs.SPITXBUF = 0; //receive temp
